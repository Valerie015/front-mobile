...
const IncidentModal: React.FC<IncidentModalProps> = ({
  ...
  const renderIncidentType = ({ item }: { item: { type: string; label: string; icon: any; color: string; image: string } }) => {
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.incidentTypeText, { color: colors.text }]}>
          {item.label}
        </Text>
      </TouchableOpacity>
    );
  ...
  return (
    <Portal>
      ...
      >
        <Text style={[styles.modalTitle, { color: colors.text }]}>Signaler un incident</Text>
        <FlatList
          data={INCIDENT_TYPES}
